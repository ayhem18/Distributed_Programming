...
def send_start_packet(sk_obj: sk.socket, server_add: tuple, seq_num: int, file_name: str, file_size: int):
    ...
    for _ in range(client_time_out_count):
        sk_obj.sendto(create_start_packet(seq_num, file_name, file_size), server_add)
        sk_obj.settimeout(client_time_out)
        server_reply, ser_add = sk_obj.recvfrom(client_buffer_size)
...
def initiate_connection(port: int, file: os.path, file_name_server: str):
    ...
    server_reply = send_start_packet(client_s, server_addr, seq_num, file_name_server, file_size)
...
def transfer(session_info: dict):
    ...
    # retrieve the necessary information about the started session

...
def transfer(port: int, file: os.path, file_name_server: str):
    global client_s
    ...
    with open(file, 'r') as f:  # this will guarantee closing the file
        for i in range(n_trans):
            ...
            server_reply = send_data_packet(server_addr, next_seq, data_bytes.encode())  # send the packet
            if server_reply is None:
                print(server_error_msg)
                sys.exit()
            else:
...
def send_data_packet(server_add: tuple, seq_num: int, data: bytes):
    for _ in range(client_time_out_count):
